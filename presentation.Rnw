...
Secretory signal peptides:
        \begin{itemize}[<+->]
          \item are short (20-30 residues) N-terminal amino acid sequences forming $\alpha$-helices,
            \item direct a protein to the endomembrane system and next to extra- or intracellular localizations,
...
\begin{frame}{Bi-partite transit signal}
The \textbf{signal peptide} is required for targeting the apicoplast protein to the endomembrane system, whereas the \textbf{transit peptide} is required to traffic the protein to the apicoplast.
...
\begin{frame}
Since amino acid composition of signal peptides differ between Plasmodium sp. and other eukaryotes, predictors of signal peptides do not predict malarial signal peptides accurately.
...
signalHsmm-2010 (hom. 50\%)                                                   & \textbf{0.7621} & \textbf{0.9384} \\
\rowcolor[gray]{0.85}signalHsmm-2010 (full alphabet)                           & 0.6853            & 0.8718            \\
\hline
\end{tabular}
\end{table}

signalHsmm-2010: trained on data set of 3676 eukaryotic proteins with signal peptides added before year 2010.
...
signalHsmm allows sensitive scanning of malarial proteome for potential drug targets.
...
signalHsmm web-server \url{http://smorfland.uni.wroc.pl/shiny/signalHsmm}.

signalHsmm R package \url{https://CRAN.R-project.org/package=signalHsmm}.
